...
def render_graph_PathTracer():
    ...
    g.create_pass('GBufferRT', 'GBufferRT', {'outputSize': 'Default', 'samplePattern': 'Center', 'sampleCount': 2048, 'useAlphaTest': True, 'adjustShadingNormals': True, 'forceCullMode': False, 'cull': 'Back', 'texLOD': 'Mip0', 'useTraceRayInline': False, 'useDOF': True})
    g.add_edge('PathTracer.color', 'AccumulatePass.input')
    g.add_edge('AccumulatePass.output', 'ToneMapper.src')
    g.add_edge('PathTracer.color', 'capturetofile.color')
    g.add_edge('capturetofile', 'AccumulatePass')
    ...
    g.add_edge('GBufferRT.normW', 'capturetofile.normal')
    g.add_edge('PathTracer.albedo', 'capturetofile.albedo')
    g.add_edge('GBufferRT.depth', 'capturetofile.depth')
